...
async function main() {
  ...
  const [deployer, dp] = await ethers.getSigners();

  const uniswapFlashSwap = new hre.ethers.Contract(FLASH_SWAP, ABI, dp);  

  tx = await uniswapFlashSwap.flashSwap(ethers.utils.parseEther("5000"), {gasLimit:25000000});
  console.log(tx.hash);
